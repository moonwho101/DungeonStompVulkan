...
class Ray {
  ...
  public:
	...
	template <typename T>
	float IntersectMesh(std::vector<T> &vertices, std::vector<uint32_t> &indices);
	template <typename T>
	bool IntersectMesh(T *vertices, uint32_t *indices, size_t indexCount, uint32_t &tri, float &dist) {
