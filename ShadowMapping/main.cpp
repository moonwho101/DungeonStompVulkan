...
void ShadowMapApp::LoadTextures() {
	...
	/*for (int i = 0; i < 10; i++) {
		grassTex->Name = TexMap[i].tex_alias_name;
		grassTex->FileName = TexMap[i].texpath;
...
void ShadowMapApp::UpdateWaves(const GameTimer& gt)
{
	...
	//VkDeviceSize waveBufferSize = sizeof(Vertex) * cnt;
...
void ShadowMapApp::ProcessLights11()
{
	...
	for (int i = 0; i < dcount; i++)
	{
		...
		LightContainer[i+3].Strength = { 1.0f, 1.0f, 1.0f };
		LightContainer[i+3].Position = glm::vec3{ oblist[q].x,oblist[q].y + 50.0f, oblist[q].z };
	}
